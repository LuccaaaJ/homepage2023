...
<ul>
	<c:choose>
		...
		<c:when test="${step eq '6'}">
			<li>
				 <h3>c:forEach Tag : 기본 반복 태크, 고정 된 횟수 또는 초과 수집 동안 중첩 된 본문 내용을 반복</h3>
				 <h4>기본문법 : <br/>
		...
		<c:when test="${step eq '12'}">
			<li>
				...
				<c:if test="${fn:startsWith(String,'JSP')}">
		...
		<c:when test="${step eq '16'}">
				<li>
					 <h3>fn:toUpperCase() : 문자열의 모든 문자를 소문자로 변환</h3>
			  		<c:set var="string" value="Welcome to JSP programming"/>
		...
		<c:when test="${step eq '20'}">
				<li>
					 <h3>fmt:formatData : 제공된 패턴 및 스타일을 사용하여 시간 및 날짜를 형식화함</h3>
